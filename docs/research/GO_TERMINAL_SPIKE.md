...
> historical evidence. Production code selectively promotes only the proven
> primitives — `internal/terminal` (headless model + host color services),
> `internal/pty` (PTY runner + zero-viewer probe), the regression tests, and
> `testdata/codex-startup.raw`. Research-only machinery (Node oracle, perf
> probes, spike CLI) remains on the spike branches and is not a production
> dependency.
