...
import (
	...
	"github.com/rceman/reposuite-relay/internal/terminal"
)
...
func buildProbe(t *testing.T) string {
	...
	out, err := exec.Command("go", "build", "-o", bin,
		"github.com/rceman/reposuite-relay/internal/pty/testprobe").CombinedOutput()
	if err != nil {
...
func TestZeroViewerQueryReply(t *testing.T) {
	...
	p, err := pty.Start(bin, nil, 120, 30, nil, "")
	if err != nil {
	...
	raw := &rawCollector{}
	m := terminal.New(terminal.Options{
	...
	fp := terminal.TakeFingerprint(m.Term)
...
func TestPtyWinsizeAtSpawn(t *testing.T) {
	p, err := pty.Start("stty", []string{"size"}, 133, 41, nil, "")
	if err != nil {
...
func TestPtyChurn(t *testing.T) {
	...
	base := countFds()
	for i := 0; i < 50; i++ {
		p, err := pty.Start("true", nil, 80, 24, nil, "")
		if err != nil {
