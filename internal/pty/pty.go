// Package pty is a minimal Linux PTY runner: a child process
...
func (p *Process) Resize(cols, rows uint16) error {
	return creackpty.Setsize(p.Ptmx, &creackpty.Winsize{Cols: cols, Rows: rows})
}

// Size reports the current PTY window size.
func (p *Process) Size() (cols, rows int, err error) {
	return creackpty.Getsize(p.Ptmx)
}
