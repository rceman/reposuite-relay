...
| 23 | Shared runtime + isolation | covered by gate 5 — several sessions share one app-server process with distinct native threads, deleting one detaches only its thread (runtime survives, siblings keep their exact native identity), runtime death projects every bound session COLD and a fresh generation resumes each exact thread |
